      ...
      <Card className="overflow-hidden group hover:shadow-xl transition-all duration-300 flex flex-col h-full border border-gray-200 bg-white">
        ...
        >
          ...
          <div className="absolute top-2 left-2 flex gap-1.5 flex-wrap">
            <Badge variant="yellow" className="shadow-md text-[10px] sm:text-xs px-2 py-0.5 font-bold">
              {property.approval}
            </Badge>
            <Badge variant="default" className="bg-blue-950 text-white shadow-md text-[10px] sm:text-xs px-2 py-0.5">
              {property.type}
            </Badge>
          </div>
          ...
          {property.status && (
            <div className="absolute bottom-2 right-2">
              <Badge variant="secondary" className="bg-black/75 text-white backdrop-blur-sm border-none text-[10px] px-2 py-0.5">
                {property.status}
              </Badge>
            </div>
          )}
        ...
        <CardContent className="p-3 sm:p-5 flex flex-col flex-grow justify-between space-y-3">
          ...
          <div className="cursor-pointer" onClick={() => setModalOpen(true)}>
            <div className="flex flex-col sm:flex-row justify-between items-start mb-1 gap-1">
              <h3 className="text-sm sm:text-xl font-bold text-blue-950 group-hover:text-blue-700 transition-colors line-clamp-1">
                {property.name}
              </h3>
              <span className="text-xs sm:text-lg font-extrabold text-blue-900 shrink-0">
                {property.price}
              </span>
            </div>

            <p className="flex items-center text-xs sm:text-sm text-gray-600 mb-2 font-medium">
          ...
          <div className="grid grid-cols-2 gap-1.5 pt-2 border-t border-gray-100">
            <Link to={`/properties/${property.id}`} className="w-full">
              <Button variant="outline" size="sm" className="w-full text-blue-950 border-blue-950 hover:bg-blue-50 font-bold text-xs px-1 py-1.5 h-8 sm:h-9">
                View Details
              </Button>
            </Link>
            ...
            <Button
              size="sm"
              variant="default"
              className="w-full bg-emerald-600 hover:bg-emerald-700 text-white font-bold text-xs px-1 py-1.5 h-8 sm:h-9"
              onClick={() => openPopup({ propertyType: property.type, location: property.location })}
            >
              <WhatsAppIcon className="h-3.5 w-3.5 mr-1 shrink-0" />
              Enquire
            </Button>
          </div>
